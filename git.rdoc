...
If you do +git pull+ and get the annoying "You asked me to pull without telling me which branch..." message, assuming you're on the +master+ branch pulling from a remote named +origin+:
...
Add with -p/--patch and you'll be prompted to add on a hunk-by-hunk basis:
...
Add the new remote, fetch it, and create a branch to track it:
  ...
  git remote add walrus git://...
  git fetch walrus
  git branch walrus-fork walrus/master

You can now checkout the new branch, explore, and merge into your master:

  git merge walrus-fork
