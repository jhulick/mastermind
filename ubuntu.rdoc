...
Install various commonly-used packages:
  ...
  sudo apt-get install ubuntu-restricted-extras rar p7zip ntp xchm inkscape \
    compizconfig-settings-manager transmission hamster-applet xclip tofrodos ack-grep
...
=== Development Basics
  ...
  sudo apt-get install build-essential gedit-plugins meld rpl curl \
    ...
    redis-server w3c-markup-validator librsvg2-bin
...
Then be sure to follow the instructions for modifying <tt>.bashrc</tt>.
  ...
  gem install rails mysql ruby-debug mongrel \
    autotest-rails capistrano jekyll hirb wirble
