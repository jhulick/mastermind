...
Firstly, to move min/max/close buttons back to the right side of the title bar:

* Launch the Configuration Editor (Alt+F2, then type 'gconf-editor').
* Change the value at app > metacity > general > button_layout to:
...
Install various commonly-used packages:
  ...
  sudo apt-get install ubuntu-restricted-extras rar p7zip ntp xchm inkscape \
...
=== Development Basics
  ...
  sudo apt-get install build-essential gedit-plugins meld rpl curl \
    subversion git-core imagemagick libxml2-dev libxslt1-dev \
    redis-server w3c-markup-validator
...
Then be sure to follow the instructions for modifying <tt>.bashrc</tt>.
  ...
  sudo apt-get install bison libssl-dev libreadline5-dev libreadline6-dev
  rvm install 1.8.7

	gem install rails mysql ruby-debug mongrel autotest-rails capistrano jekyll

The following used to be necessary but don't seem to be anymore:

  sudo apt-get install libmysql-ruby1.8 libmysqlclient15-dev libopenssl-ruby

=== Passenger

  gem install passenger
	~/.rvm/gems/ruby-1.8.7-p249/bin/passenger-install-apache2-module
