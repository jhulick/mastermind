...
Install web server and basic tools like logrotate, curl, Git, ImageMagick, etc:

  sudo apt-get -y install build-essential imagemagick logrotate monit \
...
Modify /opt/nginx/conf/nginx.conf as directed by installation script.

Start on system startup vi Upstart:
...
In addition to configuring a virtual host for each web application, log rotation should be configured. Add /etc/logrotate.d/[app_name]:
