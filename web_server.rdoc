...
Install web server and basic tools like logrotate, curl, Git, ImageMagick, etc:
  ...
  sudo apt-get -y install imagemagick logrotate monit \
    build-essential curl git-core apache2-mpm-worker apache2-threaded-dev \
    redis-server libxml2-dev libxslt1-dev
  # redis-server is not available on Hardy, but will be on Lucid (v1.2)
...
Install:
  ...
  sudo apt-get -y install mysql-server libmysql-ruby1.8 libmysqlclient15-dev
